...
function resolve(env, envKey, optionKey, fallback, sources, name) {
  ...
  if (optionKey) {
    // Codex sets the CLAUDE_-prefixed names, not CODEX_ ones: the binary carries
    // CLAUDE_PLUGIN_ROOT and CLAUDE_PLUGIN_DATA and no CODEX_ equivalent. Accept
    // both so a future rename does not silently drop every option.
    const fromOption = nonBlank(env[`CODEX_PLUGIN_OPTION_${optionKey}`])
      ?? nonBlank(env[`CLAUDE_PLUGIN_OPTION_${optionKey}`]);
...
export function loadConfig(env = process.env) {
  const sources = {};
  const baseUrl = normalizeBaseUrl(resolve(env, "EVEROS_CODEX_BASE_URL", "BASE_URL", DEFAULT_BASE_URL, sources, "baseUrl"));
  const everosDir = resolve(env, "EVEROS_CODEX_EVEROS_DIR", "EVEROS_DIR", null, sources, "everosDir");
  const startCmdRaw = resolve(env, "EVEROS_CODEX_START_CMD", null, "everos server start", sources, "startCmd");
  const userId = resolve(
    env,
    "EVEROS_CODEX_USER_ID",
    null,
  ...
  const dataDir = resolve(
    env,
    "EVEROS_CODEX_DATA_DIR",
    null,
    nonBlank(env.CODEX_PLUGIN_DATA) ?? nonBlank(env.CLAUDE_PLUGIN_DATA) ?? path.join(home, ".everos", ".codex"),
    sources,
    "dataDir",
  );
  ...
  return {
    ...
    userId,
    projectIdOverride: resolve(env, "EVEROS_CODEX_PROJECT_ID", null, null, sources, "projectIdOverride"),
    recallTimeoutMs: boundedInt(
      env.EVEROS_CODEX_RECALL_TIMEOUT_MS,
      RECALL_DEADLINE_MS,
      RECALL_DEADLINE_MIN_MS,
      RECALL_DEADLINE_MAX_MS,
    ),
    verbose: truthy(env.EVEROS_CODEX_VERBOSE),
    debug: truthy(env.EVEROS_CODEX_DEBUG),
    dataDir,
    sources,
  };
}
