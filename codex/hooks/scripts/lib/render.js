 ...
 */
/**
 * Remove a block this plugin injected from the start of a message.
 *
 * Leading only, on purpose. On Codex `additionalContext` arrives as a
 * `developer` item and never reaches this path at all - measured - so the only
 * block that can appear inside a user message is one the user quoted
 * themselves, asking about it. Stripping that would delete their words to close
 * a loop that the role filter already closes.
 */
