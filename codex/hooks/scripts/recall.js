...
import { claimWarning, touchSession, readState, rememberPrompt } from "./lib/state.js";
...
runHook("UserPromptSubmit", async (input, ctx) => {
  ...
  // Before anything that can fail or return early: the host is telling us what
  // the user typed, and capture has no other way to know. Codex stamps its own
  // scaffolding with the same turn_id and the same role, so without this the
  // Stop hook cannot tell the user's words from the host's.
  rememberPrompt(config.dataDir, sessionId, input.turn_id, prompt);
  ...
  if (!identity.userId && claimWarning(config.dataDir, sessionId)) {
    return { systemMessage: "⚠️ EverOS: no user id could be derived — set EVEROS_CODEX_USER_ID. Personal memory is off for this session." };
  }
